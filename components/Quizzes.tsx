...
const Quizzes: React.FC<QuizzesProps> = ({ quizzes, onDelete, onEdit }) => {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <div className="bg-slate-900/60 backdrop-blur-xl border border-white/10 shadow-2xl rounded-[32px] overflow-hidden">
        <table className="min-w-full divide-y border-white/5">
          <thead className="bg-white/5">
            <tr>
              <th scope="col" className="px-6 py-3 text-start text-xs font-medium text-white/70 uppercase tracking-wider">{t('subject')}</th>
              <th scope="col" className="px-6 py-3 text-start text-xs font-medium text-white/70 uppercase tracking-wider">{t('date')}</th>
              <th scope="col" className="px-6 py-3 text-start text-xs font-medium text-white/70 uppercase tracking-wider">{t('studyMaterials')}</th>
          ...
          <tbody className="divide-y border-white/5">
            {sortedQuizzes.map(quiz => (
              <tr key={quiz.id} className="group hover:bg-white/5">
                <td className="px-6 py-4 whitespace-nowrap font-bold text-lg text-white">{quiz.subject}</td>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                  {quiz.materialsUrl ?
                    <a href={quiz.materialsUrl} target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:text-white">📎</a> : t('noMaterials')}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-end text-sm font-medium space-x-4">
                  <button onClick={() => onEdit(quiz)} className="opacity-0 group-hover:opacity-100 text-indigo-400 hover:text-indigo-300 transition-opacity">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.5L16.732 3.732z" /></svg>
                  </button>
                  <button onClick={() => onDelete(quiz.id)} className="opacity-0 group-hover:opacity-100 text-red-400 hover:text-red-300 transition-opacity">
