              ...
              <div className="flex items-center justify-between">
                <div>
                  {assignment.status === SubmissionStatus.Submitted ? (
                    ...
                    <>
                      <button className="border border-indigo-500 text-indigo-400 hover:bg-indigo-500 hover:text-white px-4 py-2 rounded-lg flex items-center gap-2 text-sm font-medium">
                        ☁️ {assignment.file ? `${t('uploaded')}: ${assignment.file.name}` : t('uploadFile')}
                      </button>
                      <input id={`file-upload-${assignment.id}`} name={`file-upload-${assignment.id}`} type="file" className="sr-only" />
                    </>
                  )}
                </div>
